...
int empaquetarArchivoUnificado(SharedContext *ctx) {
    ...
    // 0. Identificador de Implementación (1 byte / uint8_t) -> 3 para Pthreads
    uint8_t implId = HUFF_IMPLEMENTATION_ID;
    fwrite(&implId, sizeof(uint8_t), 1, out);
    ...
    uint64_t currentOffset = sizeof(uint8_t) + sizeof(uint32_t) + sizeof(uint8_t) + sizeof(uint32_t) * 256;
